...
void pass1::on_pushButton_clicked()
{
    ...
    //比较原密码
    QString sql = QString("select * from admin where 账号 = '%1'").arg(username_qj);
...
//        qDebug() << pass << p1;
        ...
        sql =  QString("update admin set 密码 = '%1' where 账号 = '%2'").arg(p3).arg(username_qj);
        if (query.exec(sql))
            QMessageBox::information(this, "成功", "修改密码成功", "确定");
    }
}

void pass1::on_pushButton_2_clicked()
{
    this->close();
}
